...
[中文文档](docs/README.zh-CN.md) · [Configuration](docs/configuration.md) · [Component reference](docs/components.md) · [Migration guide](docs/migration.md)
...
[`examples/standard.typ`](examples/standard.typ) and [`examples/wide.typ`](examples/wide.typ) render the same complete component catalog on both formal canvases. The shared source [`examples/component-catalog.typ`](examples/component-catalog.typ) covers every public visual component, so profile comparisons stay page-for-page identical as the API evolves.
